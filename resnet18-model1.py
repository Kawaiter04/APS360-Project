...
if __name__ == "__main__":
    ...
    dataset3_val = torchvision.datasets.ImageFolder(
        root=root_str + "valid", transform=transform
    )
    dataset3_train = torchvision.datasets.ImageFolder(
        root=root_str + "train", transform=transform
    )
    dataset3_test = torchvision.datasets.ImageFolder(
        root=root_str + "test", transform=transform
    )

    val3_loader = torch.utils.data.DataLoader(
        dataset3_val, batch_size=64, num_workers=0, shuffle=True
    )
    train3_loader = torch.utils.data.DataLoader(
        dataset3_train, batch_size=64, num_workers=0, shuffle=True
    )
    test3_loader = torch.utils.data.DataLoader(
        dataset3_test, batch_size=64, num_workers=0, shuffle=True
    )
